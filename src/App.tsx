...
export default function App() {
  ...
  const fetchPrevisao = async (distrito: string) => {
    ...
    setError(null);
    try {
      const response = await fetch(`${import.meta.env.VITE_APP_URL}/prever-distrito/${encodeURIComponent(distrito)}`);
      if (!response.ok) {
  ...
  return (
    <div className="min-h-screen bg-[#030303] text-slate-200 flex flex-col font-sans select-none selection:bg-red-500/10 selection:text-red-400">
      ...
      <div className="bg-red-500/5 border-b border-red-500/15 py-2 px-4 text-center text-[10px] font-bold tracking-[0.15em] uppercase text-red-500 shadow-sm flex items-center justify-center gap-2">
        ...
        <span>Aviso de Evento Climático Extremo • Monitore as Regiões Críticas e Áreas de Encosta</span>
      </div>
      ...
      <header className="border-b border-white/5 bg-[#070708]/90 backdrop-blur-md sticky top-0 z-50 px-6 md:px-12 py-3.5 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-sm font-light tracking-[0.25em] text-white uppercase font-sans">Previsão Alagamentos SP</h1>
              <span className="text-[8px] bg-white/5 border border-white/10 text-slate-400 font-mono tracking-widest uppercase font-bold px-1.5 py-0.5 rounded">V2.5</span>
            </div>
            <p className="text-[10px] uppercase tracking-wider text-slate-500 font-mono mt-0.5">Plataforma de Monitoramento e Análise Climática</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          <div className="flex flex-col items-end text-right hidden md:flex">
            <span className="text-[9px] text-slate-500 uppercase font-bold tracking-[0.15em] font-mono">SERVIÇO DE HIDROLOGIA</span>
            <div className="flex items-center gap-2 mt-0.5">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
              <span className="font-mono text-[10px] font-medium text-slate-400">{currentTime} UTC-3</span>
            </div>
          </div>
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-6 md:p-12 grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          {loading ? (
            ...
            <div id="error-card" className="bg-red-500/5 border border-red-500/15 rounded-2xl p-8 text-center flex flex-col items-center justify-center">
              <AlertTriangle className="text-red-400 mb-3" size={30} />
              <h3 className="text-xs uppercase tracking-widest font-mono text-slate-200">Erro na Sincronização</h3>
            ...
            <div id="dashboard-content" className="space-y-6 animate-fade-in">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                {/* Urban Context Box */}
                <div id="urban-infrastructure-panel" className="bg-[#09090B] border border-white/10 rounded-2xl p-6 shadow-xl flex flex-col justify-between">
                  <div>
                    <div className="flex items-center gap-2.5 mb-4">
                      <div className="p-2 rounded bg-white/5 text-slate-400 border border-white/10">
                        <BookOpen size={15} />
                      </div>
                      <h4 className="text-[10px] font-bold uppercase tracking-[0.15em] text-slate-400 font-mono">Drenagem Urbana</h4>
                    </div>
                    <p className="text-xs text-slate-300 leading-relaxed font-sans">
                      A cidade de São Paulo possui mais de <strong>200 pontos de risco crítico</strong> mapeados pela Defesa Civil, concentrados nas bacias drenadas pelos canais dos rios <strong>Tietê e Pinheiros</strong>, além de córregos ocultos (ex: Córrego Água Preta e Córrego do Ipiranga).
                    </p>
                    <p className="text-[11px] text-slate-400 leading-relaxed font-sans mt-3">
                      Os reservatórios amortecedores municipais (piscinões) atuam absorvendo grandes picos de chuva para liberação controlada após a vazão do pico principal, mitigando o escoamento desordenado sobre as superfícies pavimentadas.
                    </p>
                  </div>
      ...
      <footer className="border-t border-white/5 bg-[#030303] py-8 text-center text-[10px] font-mono text-slate-500 uppercase tracking-widest mt-6">
        <div className="max-w-7xl mx-auto px-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-5">
            <span className="hover:text-slate-300 cursor-help transition-colors">PMRR SP</span>
            <span className="hover:text-slate-300 cursor-help transition-colors">CGE SP</span>
            <span className="hover:text-slate-300 cursor-help transition-colors">DEFESA CIVIL 199</span>
          </div>
        </div>
      </footer>
